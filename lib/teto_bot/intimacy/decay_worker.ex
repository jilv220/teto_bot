defmodule TetoBot.Intimacy.DecayWorker do
  @moduledoc """
  ...
  the key `{:teto_bot, TetoBot.Intimacy.Decay}`. The core decay logic is then
  delegated to `TetoBot.Intimacy.Decay.perform_decay_check_logic/1`.
  """
  ...
  def perform(_job) do
    case load_config() do
      {:ok, config} ->
        ...
        TetoBot.Intimacy.Decay.perform_decay_check_logic(config)
  ...
  defp load_config do
    ...
    with :ok <-
           TetoBot.Intimacy.Decay.validate_positive_integer(
             inactivity_threshold,
             :inactivity_threshold
           ),
         :ok <- TetoBot.Intimacy.Decay.validate_positive_integer(decay_amount, :decay_amount),
         :ok <-
           TetoBot.Intimacy.Decay.validate_positive_integer(minimum_intimacy, :minimum_intimacy) do
      config_map = %{
